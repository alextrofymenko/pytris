...
class Shape():
    ...
    def can_rotate(self, board):
        ...
        for block in self.get_blocks():
            ...
            if (not block.can_move(board, x - block.x, y - block.y)):
                return False
...
class Tetris():
    ...
    def create_new_shape(self):
        ...
        #YOUR CODE HERE
        centre = Point(int(self.BOARD_WIDTH/2), 0)
        return self.SHAPES[random.randint(0, len(self.SHAPES) - 1)](centre)
    ...
    def do_move(self, direction):
        ...
        #YOUR CODE HERE
        dir_coords = self.DIRECTION.get(direction, (0,0))
        if (self.current_shape.can_move(self.board, dir_coords[0], dir_coords[1])):
            self.current_shape.move(dir_coords[0], dir_coords[1])
            return True
